...
class CustomSky extends Mesh {
	...
	constructor() {
		...
		const shader = CustomSky.SkyShader;
	...
	saturate(delta) {
		if (!this.isSaturated) {
			this.material.uniforms['desaturation'].value = lerp(this.material.uniforms['desaturation'].value, 0, delta / 1);
			this.material.uniforms['desaturation'].value >= 0.99 ? this.isSaturated = true : null;
		}
	}

	desaturate(delta) {
		if (this.isSaturated) {
			this.material.uniforms['desaturation'].value = lerp(this.material.uniforms['desaturation'].value, 1, delta );
			this.material.uniforms['desaturation'].value <= 0.01 ? this.isSaturated = false : null;
		}
	}
...
CustomSky.SkyShader = {
	...
	fragmentShader: /* glsl */`
		...
		void main() {
            ...
            vec4 desaturated = generic_desaturate(retColor, desaturation);
            desaturated.x+=0.07;
            desaturated.y+=0.031;
            desaturated.z+=0.04;
			gl_FragColor = vec4( desaturated.xyz + vec3(brightnessOffset), 1.0 );
